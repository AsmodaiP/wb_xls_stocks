...
def insert_data_in_table(data, target_table='stocks.xlsx'):
    ...
    for list_number in range(len(wb.sheetnames)-1):
        ...
        for row in ws.rows:
            ...
            if new_value != 0:
                ...
                row[FIRST_INDEX + datetime.now().day].fill = PatternFill(start_color="B4C6E7", end_color="B4C6E7", fill_type = "solid")
...
def update_stocks(filename):
    return  insert_data_in_table(get_counts_from_table(filename))

# 2000393474004
if __name__ == '__main__':
    insert_data_in_table(get_counts_from_table())